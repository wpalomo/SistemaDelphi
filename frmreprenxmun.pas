...
var
  frmreprenxmuni: Tfrmreprenxmuni;

implementation

{$R *.dfm}

procedure Tfrmreprenxmuni.FormShow(Sender: TObject);
begin
   fecha1.Date:=date;
   fecha2.Date:=date;
   ck_s.Checked:=true;
...
procedure Tfrmreprenxmuni.BitBtn1Click(Sender: TObject);
begin
if ck_s.Checked then
begin
 con_conso.Close;
 con_conso.SQL.Text:='select mun,cod_proy,nom_proy,sum(totaloc),sum(totalimp) from oc where rendida="S" and (fec_del=:fec1 AND fec_al=:fec2) and mun=:mun group by mun,cod_proy order by nom_proy';
...
begin
 con_conso.Close;
 con_conso.SQL.Text:='select mun,cod_proy,nom_proy,sum(totaloc),sum(totalimp) from oc where rendida="S" and (fec_ela between :fec1 AND :fec2) and mun=:mun group by mun,cod_proy order by nom_proy';
 con_conso.Params[0].AsDate:=fecha1.Date;
 con_conso.Params[1].AsDate:=fecha2.Date;
 con_conso.Params[2].Asstring:=cb_nomun.Text;
 con_conso.Open;
...
procedure Tfrmreprenxmuni.bt_salirClick(Sender: TObject);
begin
 con_conso.Close;
 close;
end;

procedure Tfrmreprenxmuni.ppLabel10Print(Sender: TObject);
begin
  ppdesde.Caption:=datetostr(fecha1.date);
  pphasta.Caption:=datetostr(fecha2.Date);
end;

procedure Tfrmreprenxmuni.ppLabel7Print(Sender: TObject);
begin
