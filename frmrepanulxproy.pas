...
var
  frmrepanuxproy: Tfrmrepanuxproy;

implementation

{$R *.dfm}

procedure Tfrmrepanuxproy.FormShow(Sender: TObject);
begin
   fecha1.Date:=date;
   fecha2.Date:=date;
   ck_s.Checked:=true;
...
procedure Tfrmrepanuxproy.BitBtn1Click(Sender: TObject);
begin
if ck_s.Checked then
begin
   con_conso.Close;
   con_conso.SQL.Text:='select cod_proy, nom_proy, sum(totaloc),sum(totalimp) from oc where anulada="S" and (fec_del=:fec1 AND fec_al=:fec2)group by cod_proy order by nom_proy';
...
begin
  ...
  begin
   con_conso.Close;
   con_conso.SQL.Text:='select cod_proy, nom_proy, sum(totaloc),sum(totalimp) from oc where anulada="S" and (fec_ela between :fec1 AND :fec2)group by cod_proy order by nom_proy';
   con_conso.Params[0].AsDate:=fecha1.Date;
   con_conso.Params[1].AsDate:=fecha2.Date;
   con_conso.Open;
   pprepconso.Print;
...
procedure Tfrmrepanuxproy.bt_salirClick(Sender: TObject);
begin
con_conso.Close;
close;
end;

procedure Tfrmrepanuxproy.ppLabel7Print(Sender: TObject);
begin
...
procedure Tfrmrepanuxproy.ppLabel22Print(Sender: TObject);
begin
