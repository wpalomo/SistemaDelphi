...
uses
  ...
  ppRelatv, ppProd, ppReport, ZAbstractDataset, ZDataset, StdCtrls,
  DBCtrls, Buttons, DB, ZAbstractRODataset;

type
  Tfrmrepspae = class(TForm)
...
procedure Tfrmrepspae.FormShow(Sender: TObject);
begin
 cb_nomproy.Clear;
   nump.Clear;
   con_proy.Close;
...
procedure Tfrmrepspae.BitBtn1Click(Sender: TObject);
begin
 con_esxproy.Close;
...
procedure Tfrmrepspae.bt_salirClick(Sender: TObject);
begin
close;
end;

procedure Tfrmrepspae.pprepesxproyStartPage(Sender: TObject);
begin
ppvariable1.Value:=0;
  ppvariable2.Value:=0;
end;
